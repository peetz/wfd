...
class WFDStorage:
    """Persist WFD data using Home Assistant storage."""
    ...
    async def async_load(self) -> None:
        ...
        self._data = self._empty_data() if data is None else migrate(data, STORAGE_VERSION)
    ...
    async def async_get_user(self, user_id: str) -> User | None:
        ...
        return deserialize_user(data) if data else None
    ...
    async def async_get_meal(self, meal_id: str) -> Meal | None:
        ...
        return deserialize_meal(data) if data else None
    ...
    async def async_set_voting_round(self, voting_round: VotingRound) -> None:
        ...
        if existing and existing["status"] in {status.value for status in (VotingRoundStatus.CLOSED, VotingRoundStatus.DECISION_GENERATED, VotingRoundStatus.RESULTS_STORED)}:
            raise ValueError("Completed voting rounds are immutable")
        self._data["rounds"][voting_round.id] = _serialize_round(voting_round)
        await self.async_save()

    async def async_mark_round_results_stored(self, voting_round: VotingRound) -> None:
        self._data["rounds"][voting_round.id] = _serialize_round(voting_round)
        await self.async_save()
    ...
    async def async_get_voting_rounds(self) -> list[VotingRound]:
        return [deserialize_round(data) for data in self._data["rounds"].values()]
    ...
    async def async_add_vote(self, vote: Vote) -> None:
        ...
        meal = self._data["meals"].get(vote.meal_id)
        if meal is None:
    ...
    async def async_has_votes(self, round_id: str, user_id: str) -> bool:
        return any(item["round_id"] == round_id and item["user_id"] == user_id for item in self._data["votes"])

    async def async_get_submitted_voter_count(self, round_id: str) -> int:
        return len({item["user_id"] for item in self._data["votes"] if item["round_id"] == round_id})

    async def async_get_votes(self, round_id: str) -> list[Vote]:
        return [deserialize_vote(item) for item in self._data["votes"] if item["round_id"] == round_id]
    ...
    async def async_get_results(self, round_id: str) -> list[RoundResult]:
        return [deserialize_result(item) for item in self._data["results"] if item["round_id"] == round_id]
...
def deserialize_round(data: dict[str, Any]) -> VotingRound:
    return VotingRound(
        ...
        meals_required=data["meals_required"], voter_ids=tuple(data["voter_ids"]),
...
def deserialize_result(data: dict[str, Any]) -> RoundResult:
    return RoundResult(
        round_id=data["round_id"], meal_id=data["meal_id"], selected=data["selected"],
        decision_score=data["decision_score"], vote_score=data["vote_score"],
        historical_score=data["historical_score"], recency_score=data["recency_score"],
        rank=data["rank"], explanation=data["explanation"],
    )
